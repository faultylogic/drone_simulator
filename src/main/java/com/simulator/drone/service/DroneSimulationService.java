...
public class DroneSimulationService {
    ...
    // ~100km offshore (due east) from Halifax, NS: 44.6488°N, 63.5752°W
    ...
    private DronePayload spawnDrone(int index, List<String> shipIds) {
        ...
        // Round-robin assignment to ships (gracefully handles zero ships)
        String shipId = shipIds.isEmpty() ? null : shipIds.get((index - 1) % shipIds.size());

        // Scatter each drone randomly within a 15km radius of the offshore center
    ...
    private void update(DronePayload drone) {
        // Move up to MAX_MOVE_METERS in a random direction each tick
