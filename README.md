# drone-simulator
A Halifax-class frigate and drone fleet simulator instrumented with **OpenTelemetry**, deployable on **Kubernetes with Kind**, with metrics flowing through **BindPlane → Dynatrace**.

This repository is ideal for:
- Local development and Dynatrace metric instrumentation testing
- Simulating a multi-ship Halifax-class frigate fleet with drone squadrons
- Generating realistic OTLP metrics for Dynatrace dashboards
...
```bash
kubectl logs deployment/drone-simulator | grep BindPlane
```
...
```
drone-simulator.kind.local → 192.168.1.97
```

Or add to `/etc/hosts` on any client machine:
```bash
echo "192.168.1.97  drone-simulator.kind.local" | sudo tee -a /etc/hosts
```
...
### Configure fleet size and BindPlane (before first deploy)
...
### Access the live map
```
http://drone-simulator.kind.local/fleet-overview/mapview.html
```

### Fleet Control API
Resize the fleet at runtime via the REST API:
```bash
curl -X PUT http://drone-simulator.kind.local/fleet-overview/api/fleet/config \
  -H "Content-Type: application/json" \
  -d '{"shipCount": 6, "dronesPerShip": 20}'
```
...
Then open: `http://localhost:8080/fleet-overview/mapview.html`
...
- `ingress-nginx` pods
- `drone-simulator` pods

### Test ingress
test the ingress of the application deployment `http://drone-simulator.kind.local/fleet-overview/mapview.html` and you should see the application load
